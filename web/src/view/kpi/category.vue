  ...
  <div>
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" :label="t('view.kpi.category.name')" prop="name" min-width="160" />
        <el-table-column align="left" :label="t('view.kpi.category.description')" prop="description" min-width="200" />
        <el-table-column align="left" :label="t('view.kpi.category.weight')" prop="weight" width="120" />
    ...
    <el-drawer v-model="drawerVisible" :before-close="closeDrawer" :show-close="false">
      <template #header>
        <div class="flex justify-between items-center">
          <span class="text-lg">{{ t('view.kpi.category.title') }}</span>
          <div>
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="160px">
        <el-form-item :label="t('view.kpi.category.name')" prop="name">
          <el-input v-model="form.name" />
        </el-form-item>
        <el-form-item :label="t('view.kpi.category.description')" prop="description">
          <el-input v-model="form.description" />
        </el-form-item>
        <el-form-item :label="t('view.kpi.category.weight')" prop="weight">
...
import { 
  createKpiCategory, updateKpiCategory, deleteKpiCategory, deleteKpiCategoryByIds, findKpiCategory, getKpiCategoryList
} from '@/api/kpiCategory'

defineOptions({ name: 'KpiCategory' })
...
const fetchTable = async () => {
  const res = await getKpiCategoryList({ page: page.value, pageSize: pageSize.value })
  if (res.code === 0) {
...
const resetForm = () => {
  form.value = { ID: undefined, name: '', description: '', weight: 0 }
}
...
const onEdit = async (row) => {
  const res = await findKpiCategory({ ID: row.ID })
  if (res.code === 0) {
    const d = res.data.reCategory
    form.value = { ID: d.ID, name: d.name, description: d.description, weight: d.weight }
    type.value = 'update'
...
const submitForm = () => {
  formRef.value.validate(async (valid) => {
    if (!valid) return
    let res
    if (type.value === 'create') res = await createKpiCategory(form.value)
    else res = await updateKpiCategory(form.value)
    if (res.code === 0) { ElMessage.success(t('general.createUpdateSuccess')); closeDrawer(); fetchTable() }
  })
}

const onDelete = (row) => async () => {
  const res = await deleteKpiCategory({ ID: row.ID })
...
const onBatchDelete = async () => {
  ...
  const res = await deleteKpiCategoryByIds({ ids })
